...
const NAV = [
  ...
  { href: "/members", label: "Состав", icon: Users },
...
function StatusBlock() {
  ...
  const Row = ({
    ...
    <div className="flex items-center gap-2.5 px-1 py-1">
      ...
      <span className="ml-auto flex items-center gap-1.5">
        <span
          className={`dot ${state === "ok" ? "dot-ok" : state === "err" ? "dot-err" : "dot-dim"} ${state === "err" ? "pulse-dot" : ""}`}
        />
  ...
  return (
    <div
      className="card mt-auto"
      style={{ padding: "12px 12px 8px", background: "rgba(255,255,255,0.025)" }}
    >
      <div className="label px-1 pb-1.5">Система</div>
      <Row
        icon={Bot}
        label="Discord-бот"
        state={status ? (status.bot.ok ? "ok" : "err") : "dim"}
        hint={
          status
            ? status.bot.ok
              ? `@${status.bot.user?.username ?? "онлайн"}`
              : "ошибка"
            : "…"
        }
      />
      <Row
        icon={Globe}
        label="rs-red.com"
        state={status ? (status.site.ok ? "ok" : "err") : "dim"}
        hint={status ? (status.site.ok ? "доступен" : "нет доступа") : "…"}
      />
      <Row
        icon={Activity}
        label="Планировщик"
        state={status ? (status.schedulerAlive ? "ok" : "err") : "dim"}
        hint={status ? (status.schedulerAlive ? "работает" : "остановлен") : "…"}
      />
    </div>
  );
}
...
export default function Shell({ children }: { children: React.ReactNode }) {
  ...
  // Добавляем логику чтения роли пользователя из cookie
  ...
  // Фильтруем меню: настройки показываются только для админов
  const filteredNav = NAV.filter(item => item.href !== "/settings" || role === "admin");

  // Служебная вкладка расширения: без сайдбара и фоновых запросов панели.
    ...
    <>
      ...
      <aside className="sidebar">
        <Link href="/" className="flex items-center gap-3 px-2 pb-6">
          <div
            className="floaty flex items-center justify-center"
            style={{
              width: 42,
              height: 42,
              borderRadius: 14,
              background:
                "linear-gradient(135deg, #ff5148, #b41d1d 70%)",
              boxShadow:
                "0 12px 28px -10px rgba(255,61,61,.65), inset 0 1px 0 rgba(255,255,255,.3)",
            }}
          >
          ...
          <div>
            ...
            <div className="text-[10px] uppercase" style={{ color: "var(--dim)", letterSpacing: "0.22em" }}>
              division&nbsp;05
            </div>
          </div>
        </Link>
        ...
        <nav className="flex flex-col gap-1">
          {/* Здесь используем filteredNav вместо NAV */}
      ...
      <div className="mobile-bar">
        <Link href="/" className="flex items-center gap-2 pr-2">
          ...
          <span className="display text-[12px] font-bold">RED OPS</span>
        </Link>
        {/* Здесь тоже используем filteredNav вместо NAV */}
        {filteredNav.map((item) => {
          const active =
            item.href === "/" ? pathname === "/" : pathname.startsWith(item.href);
          return (
